...
- 주의할 점: Estimator와 Estimate은 다른 개념
    - Estimator: statistic (the function of random variable) used to estimate something, 즉 그냥 random variable
	- Estimate: The observed value of the estimator, 즉 observed value of random variable (=data)
- 어떤 확률변수 $$X_i$$에 대한 데이터 $$(x_1, x_2, ... , x_n)$$이 주어졌을 때, 확률변수 $$X_i$$의 분포 종류 $$f$$를 알고 있고(가정하고), $$f$$의 매개변수 $$\theta$$를 알아내는 과정이 바로 MLE
...
- 평균을 모르는 두 집단을 비교할 때
	...
	- 모분산을 모르지만 같다는 걸 알 때 표본평균의 비교: t분포와 카이제곱분포
...
- point estimator $$d := d(\mathbr{X})$$가 $$\theta$$의 estimator로 충분한지 평가하려면 어떻게 하면 좋을까?
...
- **모분산을 모를 때 표본평균의 값을 검정하려 한다면, t분포를 사용해야 한다**. 이를 **t검정(t-test)**이라고 한다.
