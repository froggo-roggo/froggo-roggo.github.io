...
- 무슨 뜻이지? 결국 사후 확률도 사전 확률에 근거한 '믿음'이라는 것이다.
	...
	$$ p(\tilde{x} \in C \mid D) = \sum\limits_{h} p(\tilde{x} \in C \mid h) \ p(h \mid D) $$ (이산 분포)

	$$ p_{\theta}(\tilde{x} \mid D) = \int p_{\theta}(\tilde{x} \mid z) \ p_{\theta}(z \mid D) dz $$ (연속 분포, h = z(θ)라는 모델로 둠)
		
	- 이때 marginal likelihood를 최대화하는 z의 parameter θ: $$ \theta_{max} =  \underset{\theta}{\mathrm{argmax}} \ \int p_{\theta}(z \mid D) dz $$
	- 이부분이제일헷갈림솔직히문자가다제멋대로라서누가삼국통일좀해줬으면좋겠음
...
- 그리고 여기에 z에 대한 임의의 모델 $$ q(z \mid \theta) $$ (혹은 $$ q_{\theta}(z) $$ 라고도 쓰는 것 같음 중요한건 q가 θ에 대한 model이고 latent variable z를 modelling한다는 점임)을 도입하면
