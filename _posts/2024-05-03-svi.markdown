...
$$ \geq \int q(z \mid \theta) \cdot log(\frac{p(x, z)}{q(z \mid \theta)}) \, dz$$

- log의 장점은 곱셈, 나눗셈을 덧셈, 뺄셈으로 쉽게 분리할 수 있다는 점이다.

$$ = \int q(z \mid \theta) \ log(p(x, z)) \, dz - \int q(z \mid \theta) \ log(q(z \mid \theta)) \, dz$$

- 위 식을 **증거 하한 (evidence of lower bound, ELBO)**라고 한다.
	- 위 식에서 앞부분은 expected log likelihood $$E_{q(z \mid \theta)}(log(p(x, z)))$$, 뒷부분은 **Kullback-Leibler 발산 (KL divergence)**이다.
